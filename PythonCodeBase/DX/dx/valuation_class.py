...
class valuation_class(object):
    ...
    payoff_func : string
        derivaive payoff in Python syntax
    ...
    =======
    update :
        updates selected valuation parameter
    delta :
    ...
    def __init__(self, name, underlying, mar_env, payoff_func=''):
        try:
            ...
            # simulation parameters and discount curve from the simulation object
    ...
    def update(self, initial_value=None, volatility=None,
        ...
        if strike is not None:
            self.final_date = strike
        if maturity is not None:
            self.maturity = maturity
            # add new maturity date id not in time grid
            if not maturity in self.underlying.time_grid:
                self.underlying.special_dates.append(maturity)
                self.underlying.instrument_values = None

    def delta(self, interval=None, accuracy=4):
        # self.present_value will be defined in the derived class
        # This method calculates the present value of the derivative
        if interval is None:
            interval = self.underlying.initial_value / 50.
        # formard difference approximation
        ...
        # we need a different initial value to compute the right present value
        self.underlying.update(initial_value=initial_del)
        # calculate the right value for numerical delta
