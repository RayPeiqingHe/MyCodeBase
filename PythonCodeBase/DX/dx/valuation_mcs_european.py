...
class valuation_mcs_european(valuation_class):
    ...
    by single-factor Monte-Carolo simulation
    ...
    present_value:
        returns present value (Monte Carolo estimator)
    '''
    ...
    def generate_payoff(self, fixed_seed=False):
        ...
        # average value over whole single path
        mean_value = np.mean(paths[:time_index], axis=1)
        # maximum value over the whole single path
        max_value = np.amax(paths[:time_index], axis=1)[-1]
        # minumum value over the whole single path
    ...
    def present_value(self, accuracy=6, fixed_seed=False, full=False):
        ...
        fixed_seed : Boolean
            use same/fixed seed for valuation
        full : Boolean
            return also full id array of present value
        '''
