...
class derivatives_portfolio(object):
    ...
    val_env : instance of market_environment
    ...
    def __init__(self, name, positions, val_env, assets,
        ...
        for pos in self.positions:
            # determine earliest starting date
    ...
    def get_statistics(self, fixed_seed=False):
        ...
        for pos, value in self.valuation_objects.items():
            ...
            res_list.append([
                p.name,
                p.quantity,
                # calculate all present vaues for the single instruments
                pv,
                value.currency,
                # single instrument vaue times quantity
                pv * p.quantity,
                # calculate Delta of positiom
                value.delta() * p.quantity,
                # calculate Vega of the position
                value.vega() * p.quantity
            ])

        # gennerate a pandas DataFrame object with all results
