...
class jump_diffusion(simulation_class):
    ...
    def generate_paths(self, fixed_seed=False,day_count=365.):
        ...
        # standard normally distributed pesudorandom numbers
        ...
        for t in range(1, len(self.time_grid)):
            ...
            if self.correlated is False:
                ...
                # only with correlated in portfolio context
                ran = np.dot(self.cholesky_matrix, sn1[:, t, :])
                ran = ran[self.rn_set]
            dt = (self.time_grid[t] - self.time_grid[t - 1]).days / day_count
            # difference between two days as year fraction
            poi = np.random.poisson(self.lamb * dt, I)
            # Poisson-distributed pesudorandom numbers for jump component
